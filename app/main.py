...
def get_nearest_cities(
    city_name: str,
    max_results: Optional[int] = Query(5, gt=0, le=10)
):
    """
    ...
    nearest_cities = [
        {"name": city.decode("utf-8"), "distance": float(distance)}
    ...
    return {
        "city": city_name,
        "nearest_cities": nearest_cities[:max_results],
        "unit": "km"
    }
...
def read_root():
    return {
        "message": """Bem-vindo à API de Distância entre Cidades.
        Use /docs para ver os endpoints disponíveis."""
    }
