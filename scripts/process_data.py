...
def calculate_distances(city, cities):
    ...
    for other_city in cities:
        ...
        if city_name != other_city_name:
            ...
            distance = haversine(city_lat_lon, other_city_lat_lon)  # Distâncias em km
...
for i, city in enumerate(cities):
    ...
    # Registra o progresso a cada 5% ou ao final
    total_cities = len(cities)
    progress_interval = total_cities // 20  # 5% do total
    if (i + 1) % progress_interval == 0 or i + 1 == total_cities:
        progress = (i + 1) / len(cities) * 100
        logger.info(
            f"Progresso: {i + 1}/{len(cities)} cidades processadas ({progress:.1f}%)"
        )
